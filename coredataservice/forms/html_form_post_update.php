<?php
if (isset($_POST) && !empty($_POST) && isset($_POST['coredataservice_update_submit'])){

	// is active
	$coredataservice_apiid = sanitize_text_field($_POST['apiid']);
	...
	/** selected_columns ends **/
		...
		// save raw body if raw
		if($coredataservice_post_method_body_type=='raw'){
			$getpostfields = $coredataservice_postbodyraw;
		}else{
			$getpostfields=json_encode($getpostfields);
		}
		...
		$wpdb->update( 
			...
			array(
				...
				'cds_postfields_value' => ($getpostfields),
			...
			array('id'=>$coredataservice_apiid)
		);
		echo "<script>
